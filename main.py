...
parser.add_argument('--input_size', default=32, type=int, help='img crop size')
parser.add_argument('--image_size', default=32, type=int, help='ori img size')
...
def main():
    ...
    criterion = FocalLoss(device,10,gamma=args.fl_gamma)
...
#     lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,milestones=[1, 225], last_epoch=args.start_epoch - 1) 

    # Data loading code
    normalize = transforms.Normalize(mean=[x / 255 for x in [125.3, 123.0, 113.9]],  ##cifar 10
                                     std=[x / 255 for x in [63.0, 62.1, 66.7]])
    img_size = args.input_size

    ratio = 32.0 / float(img_size)
#     train_dataset = CASIA(
#         transforms.Compose([
...
#                                              num_workers=args.workers, pin_memory=False, sampler=val_sampler)
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10(root='./data', train=False, transform=transforms.Compose([
            transforms.Resize(int(40 * ratio)),
            transforms.CenterCrop(img_size),
            transforms.ToTensor(),
            normalize,
        ])),
